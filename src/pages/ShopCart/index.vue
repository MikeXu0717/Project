  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck" @change="updateAllCartChecked" :disabled='cartInfoList.length==0'/>
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  computed: {
    ...
    //计算购买产品的总价
    totalPrice() {
      var checked = this.cartInfoList.filter((cart) => {
        return cart.isChecked == 1;
      });
      let sum = checked.reduce((total, current) => {
        return total + current.skuNum * current.skuPrice;
      }, 0);
      return sum;
    },
