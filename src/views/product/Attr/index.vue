  ...
  methods: {
    ...
    updateAttr(row) {
      ...
      this.attrInfo.attrValueList.forEach((item) => {
        ...
        // 因为Vue无法探测普通的新增 property,这样书写的属性并非响应式属性
        // 第一个参数:对象  第二个参数:添加的新响应式属性  第三参数：新属性的属性值
        this.$set(item, "flag", false);
      });
    },
    //失去焦点的事件---切换为查看模式，展示span
    toLook(row) {
      // 如果属性值为空需要给用户提示，让他输入一个其他的属性值
    ...
    toEdit(row, index) {
      ...
      //点击span的时候，重绘重排一个input需要耗费时间，因此我们不可能一点击span立马获取到input
