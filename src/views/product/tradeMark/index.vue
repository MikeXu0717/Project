<template>
  <div>
    ...
    <!-- 
      分页器 
      需要知道：当前第几页、数据总条数、每一页展示条数、连续页码数

      current-page:代表的是当前第几页
      total：代表分页器一共需要展示数据条数
      page-size：代表的是每一页需要展示多少条数据
      page-sizes：代表可以设置每一页展示多少条数据
      layout：可以实现分页器布局  ->去最右边
      pager-count:按钮的数量  如果 9  连续页码是7
      @size-change="handleSizeChange":每一页展示数据条数改变时会触发
    ...
    <el-dialog :title="tmForm.id ? '修改品牌' : '添加品牌'" :visible.sync="dialogFormVisible">
      <!-- form表单 model:把表单的数据收集到哪个对象的身上 ，将来表单验证，也需要这个属性-->
...
export default {
  ...
  data(){
    ...
    return{
      ...
      rules: {
        // 品牌名称的验证规则
        // require:是否必须要校验的字段（与前面的五角星有关系）  message 提示信息   trigger:用户行为设置（事件的设置:blur、change）
        tmName: [
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      // 获取品牌列表的接口
      // 当你向服务器发请求的时候，这个函数需要带参数，因此老师在data当中初始化两个字段，代表给服务器传递的参数
    ...
    updateTradeMark(row) {
      ...
      // 将服务器返回品牌的信息，直接赋值给了tmForm进行展示。也就是tmForm存储的即为服务器返回品牌信息
      // this.tmForm = row;
      this.tmForm = { ...row };
    },
    // 图片上传成功
